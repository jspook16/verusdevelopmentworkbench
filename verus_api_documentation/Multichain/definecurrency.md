...
**Cost of Definition:**
It's important to note that defining a currency on the Verus mainnet incurs a cost, currently 200 VRSC. This fee is for the `definecurrency` transaction itself, which registers the currency definition on the blockchain. This cost is separate from any funds contributed for pre-conversion or initial reserves. As such, this 200 VRSC fee is non-refundable and covers the network resources used to process and store the currency definition, regardless of whether the currency subsequently meets its launch criteria or if pre-conversion deposits (which are generally refundable in case of a failed launch) are involved. On test networks like VRSCTEST, a similar cost structure applies but is paid in test coins with no real-world value. Standard network transaction fees for the `definecurrency` transaction are also applicable in addition to this definition cost.
