...
// Re-using the generic key-value renderer
...
const IdentitiesWithRevocationForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await sendCommand('getidentitieswithrevocation', params, 'query');
    } catch (err) {
      console.error('Error in IdentitiesWithRevocationForm:', err);
    }
  };
  ...
  return (
    <Paper sx={{ p: 2, background: '#181818' }}>
      <Typography variant="h6" sx={{ color: '#fff', mb: 2 }}>Identities With Revocation Authority</Typography>
      <Typography variant="body2" sx={{ color: '#bbb', fontStyle: 'italic', mb: 2 }}>
        Find identities that have the specified identity as their revocation authority.
      </Typography>
      <Box component="form" onSubmit={handleSubmit} sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
        <TextField
          label="Revocation Authority Identity ID *"
          value={identityId}
          onChange={(e) => setIdentityId(e.target.value)}
          placeholder="Enter i-address of the revocation authority"
          required
          fullWidth
        />
        <TextField
...
export default IdentitiesWithRevocationForm; 