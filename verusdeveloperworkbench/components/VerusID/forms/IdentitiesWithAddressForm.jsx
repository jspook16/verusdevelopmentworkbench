...
const IdentitiesWithAddressForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!address.trim()) {
      opCtx.setOperationError('Address is required.');
    ...
    try {
      await sendCommand('getidentitieswithaddress', params, 'query');
    } catch (err) {
      console.error('Error in IdentitiesWithAddressForm:', err);
  ...
  return (
    <Paper sx={{ p: 2, background: '#181818' }}>
      <Typography variant="h6" sx={{ color: '#fff', mb: 2 }}>Identities By Address</Typography>
      ...
      <Box component="form" onSubmit={handleSubmit} sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
        ...
        <FormControlLabel
          ...
          label="Include Watch-Only Addresses"
          sx={{color: '#fff'}}
        />
        ...
        <Box sx={{ mt: 3, p: 2, background: '#232323', borderRadius: 1, maxHeight: '60vh', overflowY: 'auto' }}>
          ...
          {Array.isArray(currentResult) && currentResult.length > 0 ? (
            currentResult.map((item, index) => (
              // Assuming item structure is similar to listidentities items for display purposes
            ...
            <Typography sx={{ color: '#bbb', fontStyle: 'italic' }}>
              No identities found for the specified address.
            </Typography>
          ) : (
            <Typography sx={{ color: '#bbb', fontStyle: 'italic' }}>
...
export default IdentitiesWithAddressForm; 