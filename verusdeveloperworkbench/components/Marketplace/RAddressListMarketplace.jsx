...
import { Box, Typography, List, ListItemButton, CircularProgress, IconButton, Tooltip, Button, Switch, FormControlLabel } from '@mui/material';
...
/**
 * RAddressListMarketplace - Displays a list of R-Addresses with balance information
 * 
 ...
 */
const RAddressListMarketplace = ({ contextType = 'marketplace', hideHeader = false, hideHeaderTitle = false }) => {
  ...
  switch (contextType) {
    case 'vdxf':
      ...
      handleSelectAddressForContext = idContext.selectRAddressForVDXF;
      break;
    case 'currency':
      selectedAddressFromContext = idContext.selectedRAddressForCurrencyWS;
      handleSelectAddressForContext = idContext.selectRAddressForCurrencyWS;
      break;
    case 'marketplace':
    default:
      selectedAddressFromContext = marketplaceContext.selectedRAddress;
      handleSelectAddressForContext = marketplaceContext.setSelectedRAddress;
      break;
  }
  ...
  const filteredRAddressData = useMemo(() => {
    ...
    if (hideZeroBalanceRAddrs) {
      return rAddressData.filter(rAddr => {
        ...
        let isPrimaryForID = identities && identities.some(id => id.identity.primaryaddresses && id.identity.primaryaddresses.includes(rAddr.address));
  ...
  const handleRAddressSelect = (rAddress) => {
    handleSelectAddressForContext(rAddress);
  };
      ...
      {!hideHeader && (
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', p: 1, backgroundColor: '#191919', flexShrink: 0 }}>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', ml: hideHeaderTitle ? 'auto' : 0 }}>
            ...
            {hideZeroBalanceRAddrs && rAddressData && (
              <Typography sx={{ fontSize: '0.75rem', color: '#aaa', minWidth: 24, textAlign: 'center' }}>
                {rAddressData.length - filteredRAddressData.length}
              </Typography>
            )}
          </Box>
        </Box>
        ...
        {filteredRAddressData && filteredRAddressData.length > 0 && (
          <List dense disablePadding sx={{pt:0.5}}>
            {filteredRAddressData.map((rAddr) => {
              const isSelected = selectedAddressFromContext === rAddr.address;
                  ...
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', width: '100%', alignItems: 'center' }}>
                    ...
                    <Box sx={{ flexGrow: 1, overflow: 'hidden', mr: 0.5 }}>
                      ...
                      <Box sx={{ display: 'flex', alignItems: 'center', width: '100%', mt: -0.5 }}>
                        ...
                        {identities && identities.some(id => id.identity.primaryaddresses && id.identity.primaryaddresses.includes(rAddr.address)) && (
                    ...
                    <Tooltip title={`Copy R-Address: ${rAddr.address}`}>
                      <IconButton 
                        ...
                        aria-label={`Copy R-Address ${rAddr.address}`}
                      >
...
export default RAddressListMarketplace; 